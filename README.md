...
Automation Script
The deployment process is automated using a Bash script named deploy-webserver.sh.
...
• Accessing the server IP address in a web browser

Monitoring
Basic monitoring can be performed using system logs and service status commands.
Administrators can check logs and service status to ensure the web server is running correctly.
